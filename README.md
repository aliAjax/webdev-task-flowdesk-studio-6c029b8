...
[`trajectory.jsonl`](./trajectory.jsonl) 是原始执行事件流，包含会话元数据、
用户需求、助手回复、工具调用与返回、补丁事件、任务完成信息和 token 统计，
是执行轨迹的权威来源。可使用以下命令计算文件的 SHA-256：

```bash
shasum -a 256 trajectory.jsonl
```

文件采用 JSON Lines 格式，每行是一条带时间戳的独立 JSON 事件，可使用
`jq` 等结构化工具解析。没有公开摘要的加密推理事件不应被推测或还原；审计
时应直接依据 `trajectory.jsonl` 中的原始字段。
